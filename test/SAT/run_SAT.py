...
def run_sts_solver(n: int, active_constraints: list[str], encoding_type: str = "bw", timeout_sec: int = 300, verbose: bool = False) -> dict:
    """
    Runs the STS SAT solver with the specified configuration.
    ...
    # Configure constraint flags based on active_constraints
    constraints = {
    ...
    try:
        # Call the solve_sts function from the sts module with encoding type
        ...
        if result['satisfiable']:
            return {
                ...
                "optimal": "true",
                "obj": None,  # SAT doesn't have optimization objective
                "sol": result['solution'],
                "solver": "z3",
...
def run_sts_with_averaging(n: int, active_constraints: list[str], encoding_type: str = "bw", timeout_sec: int = 300,
                          verbose: bool = False, num_runs: int = 5, max_workers: int = None) -> dict:
    """
    ...
    for i in range(num_runs):
        try:
            result = run_sts_solver(n, active_constraints, encoding_type, timeout_sec, verbose=False)
...
def run_test_mode(n: int, constraints: list[str], encoding_type: str = "bw", timeout_sec: int = 300,
                 verbose: bool = False, num_runs: int = 5, max_workers: int = None):
    """
...
def run_select_mode(n: int, group_name: str, encoding_type: str = "bw", timeout_sec: int = 300, 
    ...
    """
    if group_name == "all":
        # Run each constraint group independently
        all_results = []
        all_names = []
        ...
        for group in ALL_CONSTRAINTS_GROUPS.keys():
            ...
            group_results, group_names = run_select_group(
                n, group, encoding_type, timeout_sec, verbose, num_runs, max_workers
            )
        ...
        return run_select_group(n, group_name, encoding_type, timeout_sec, verbose, num_runs, max_workers)
...
def run_select_group(n: int, group_name: str, encoding_type: str = "bw", timeout_sec: int = 300,
    ...
    results, names = run_test_mode(n, group_constraints, encoding_type, timeout_sec, verbose, num_runs, max_workers)
...
Examples:
  ...
  python run_STS.py 8 --test --constraints use_symm_break_weeks use_symm_break_teams --encoding seq
  python run_STS.py 10 --select symm --timeout 600 --encoding np
        """
    )
    ...
    if args.test:
        ...
        results, names = run_test_mode(
            args.teams, args.constraints, args.encoding, args.timeout, args.verbose, args.runs, args.max_workers
        )
    elif args.select:
        # Select group mode: run with all possible combinations of the selected constraint group
        ...
        results, names = run_select_mode(
            args.teams, args.select, args.encoding, args.timeout, args.verbose, args.runs, args.max_workers
        )
    else:
